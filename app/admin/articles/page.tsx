...
export default async function AdminArticles() {
  ...
  return (
    <div>
      <PageHeader
        title="Articles"
        actionHref="/admin/articles/nouveau"
      ...
      <div className="overflow-hidden rounded-xl border border-marine/10 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {articles.map((a) => (
              <tr key={a.id} className="border-b border-marine/5 last:border-0">
                <td className="px-4 py-3 font-medium text-marine">
                  <Link href={`/admin/articles/${a.id}`} className="hover:text-bleu">
                    {a.titre}
                  </Link>
                </td>
                ...
                <td className="px-4 py-3 text-marine/70">{a.vues}</td>
            ...
            {articles.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-marine/50">
